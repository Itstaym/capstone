async function getAllBooks() {
      ...
      myHeaders.append("Authorization", "Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ0ZXN0QHRlc3QudGVzdCIsImlhdCI6MTc1OTY1NDQ1NywiZXhwIjoxNzU5NjU1MzU3fQ.w1QJWtQiIruO_1mUG9NwBrkcx0gl8x1A17GmnuXl6Gk");
      ...
      await fetch("http://localhost:8080/api/books", requestOptions)
        .then((response) => response.text())
        .then((result) => data = JSON.parse(result))
        .catch((error) => console.error(error));

      data.forEach(element => {
        addBookToTable(element.title, element.author, element.edition, element.publisher, element.year);  
      });
    }
...
/*
    ...
    function deleteRow(btn) {
      if (confirm("Are you sure you want to delete this book?")) {
        const table = document.getElementById("booksTable");
        const row = btn.closest("tr");
        const deletedIndex = row.rowIndex;
        
        // Delete the row
        row.remove();
        
        // Update accession numbers for all rows after the deleted row
        const rows = table.getElementsByTagName("tr");
        for (let i = deletedIndex; i < rows.length; i++) {
            // Skip header row
            if (i === 0) continue;
            
            // Format new accession number
            const newAccessionNum = i.toString().padStart(4, '0');
            
            // Update accession number in the first cell
            rows[i].cells[0].innerHTML = newAccessionNum;
            
            // Update QR code content with new accession number
            const title = rows[i].cells[1].innerText;
            const author = rows[i].cells[2].innerText;
            const edition = rows[i].cells[3].innerText;
            const publisher = rows[i].cells[4].innerText;
            const year = rows[i].cells[5].innerText;
            
            // Create new QR code with updated accession number
            const qrContainer = rows[i].cells[6];
            qrContainer.innerHTML = "";
            ...
            const qrContent = `Accession Number: ${newAccessionNum} | Title: ${title} | Author: ${author} | Edition: ${edition} | Publisher: ${publisher} | Year: ${year}`;
            new QRCode(qrDiv, {
                text: qrContent,
                width: 110,
                height: 110,
